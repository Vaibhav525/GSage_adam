...
if __name__ == '__main__':
	...
	unsupervised_loss = [UnsupervisedLoss(getattr(dataCenter, ds+'_adj_lists')[i], getattr(dataCenter, ds+'_train')[i], device) for i in range(Val_i)]
	unsupervised_loss_val = [UnsupervisedLoss(getattr(dataCenter_val, ds+"_val"+'_adj_lists')[i], getattr(dataCenter_val, ds+"_val"+'_train')[i], device) for i in range(0,N-Val_i,1)]
	...
	for epoch in range(args.Val):
		#create a loop here#
		print('----------------------EPOCH %d-----------------------' % epoch)
		graphSage, classification = apply_model(dataCenter_val, ds+"_val", graphSage, classification, unsupervised_loss_val, args.b_sz, args.unsup_loss, device, args.learn_method)
		#if (epoch+1) % 2 == 0 and args.learn_method == 'unsup':
		#	classification, args.max_vali_f1 = train_classification(dataCenter, graphSage, classification, ds, device, args.max_vali_f1, args.name)
		if args.learn_method != 'unsup':
			args.max_vali_f1 = evaluate(dataCenter, ds+"_val", graphSage, classification, device, args.max_vali_f1, args.name, epoch)

		nodes = getattr(dataCenter_val, ds+"_val"+'_train')[epoch]
		features = torch.FloatTensor(getattr(dataCenter_val, ds+"_val"+'_feats')[epoch]).to(device)
		adj_list=getattr(dataCenter_val, ds+"_val"+'_adj_lists')[epoch]
		Final_embs=graphSage(np.asarray(nodes),features,adj_list)
		t_np = Final_embs.detach().cpu().numpy() #convert to Numpy array
		df = pd.DataFrame(t_np) #convert to a dataframe
		df.to_csv("Fin_embs_"+str(Val_i+epoch)+".csv",index=False) #save to file

	torch.save(graphSage,'models/Final_model.torch')
